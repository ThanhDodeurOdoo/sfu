            ...
            proms.push(
                (async () => {
                    try {
                        ...
                        bitRates[type as StreamType] = bitRate;
                    } catch (error) {
        ...
        for (const [type, producer] of Object.entries(session.producers)) {
            ...
            try {
                ...
                if (!consumer) {
                    ...
                    consumers[type as StreamType]?.close();
                    consumers[type as StreamType] = newConsumer;
                }
                await this._syncConsumerState(consumers[type as StreamType]!, producer);
            } catch (error) {
                consumers[type as StreamType]?.close();
                consumers[type as StreamType] = null;
    ...
    private async _handleMessage({ name, payload }: BusMessage): Promise<void> {
        switch (name) {
            ...
            case CLIENT_MESSAGE.CONSUMPTION_CHANGE: {
                const { sessionId, states } = payload as {
                    sessionId: SessionId;
                    states: Record<StreamType, boolean>;
                };
                for (const [type, active] of Object.entries(states)) {
                    const consumer = this._consumers.get(sessionId)?.[type as StreamType];
                    if (consumer) {
                        logger.verbose(
            ...
            case CLIENT_MESSAGE.PRODUCTION_CHANGE: {
                ...
                const producer = this.producers[type as StreamType];
                if (!producer) {
                    return;
                }
            ...
            case CLIENT_MESSAGE.INFO_CHANGE: {
                const { info, needRefresh } = payload as {
    ...
    private async _handleRequest({ name, payload }: BusMessage): Promise<JSONSerializable | void> {
        switch (name) {
            case CLIENT_REQUEST.CONNECT_STC_TRANSPORT: {
                const { dtlsParameters } = payload as { dtlsParameters: DtlsParameters };
            ...
            case CLIENT_REQUEST.INIT_PRODUCER: {
                const { type, kind, rtpParameters } = payload as {
                    type: StreamType;
                    kind: MediaKind;
                    rtpParameters: RtpParameters;
                };
                this.producers[type as StreamType]?.close();
                this.producers[type as StreamType] = null;
                ...
                this.producers[type as StreamType] = producer;
                this.on("close", () => {
                    producer.close();
                    this.producers[type as StreamType] = null;
                });
