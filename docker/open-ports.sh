...
echo -e "${GREEN}Opening required ports for Odoo SFU${NC}"
...
# Allow SFU port if direct access is needed
echo -e "- Allowing SFU port $SFU_PORT"
ufw allow $SFU_PORT/tcp

# Allow RTC port ranges
...
echo -e "\n${YELLOW}Important: Ensure your cloud provider or VPS panel also allows these ports in their network firewall settings.${NC}"